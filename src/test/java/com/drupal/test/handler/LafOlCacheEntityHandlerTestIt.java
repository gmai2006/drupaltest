...
import com.drupal.test.dao.DefaultLafOlCacheEntityDao;
...
public class LafOlCacheEntityHandlerTestIt {
  static final String inputFile = "LafOlCacheEntity.json";
  static LafOlCacheEntityHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private LafOlCacheEntity[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCacheEntity.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCacheEntity[].class);
    assertEquals("match count", count, records.length);
    LafOlCacheEntity testResult = jpa.find(LafOlCacheEntity.class, records[0].getCid());
