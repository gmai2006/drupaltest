...
import com.drupal.test.dao.DefaultLafOlCacheDefaultDao;
...
public class LafOlCacheDefaultHandlerTestIt {
  static final String inputFile = "LafOlCacheDefault.json";
  static LafOlCacheDefaultHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private LafOlCacheDefault[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCacheDefault.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCacheDefault[].class);
    assertEquals("match count", count, records.length);
    LafOlCacheDefault testResult = jpa.find(LafOlCacheDefault.class, records[0].getCid());
