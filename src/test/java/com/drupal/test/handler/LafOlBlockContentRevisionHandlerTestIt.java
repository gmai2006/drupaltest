...
import com.drupal.test.dao.DefaultLafOlBlockContentRevisionDao;
...
public class LafOlBlockContentRevisionHandlerTestIt {
  static final String inputFile = "LafOlBlockContentRevision.json";
  static LafOlBlockContentRevisionHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlBlockContentRevision.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlBlockContentRevision[].class);
    assertEquals("match count", count, records.length);
    LafOlBlockContentRevision testResult =
        jpa.find(LafOlBlockContentRevision.class, records[0].getRevisionId());
