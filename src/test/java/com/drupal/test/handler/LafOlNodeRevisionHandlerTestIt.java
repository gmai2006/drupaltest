...
import com.drupal.test.dao.DefaultLafOlNodeRevisionDao;
...
public class LafOlNodeRevisionHandlerTestIt {
  static final String inputFile = "LafOlNodeRevision.json";
  static LafOlNodeRevisionHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlNodeRevision.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlNodeRevision[].class);
    assertEquals("match count", count, records.length);
    LafOlNodeRevision testResult = jpa.find(LafOlNodeRevision.class, records[0].getVid());
    ...
    org.junit.Assert.assertEquals(
        ...
        this.records[0].getRevisionUid(),
        testResult.getRevisionUid());
    org.junit.Assert.assertEquals(
        "expect equals revisionTimestamp ",
        this.records[0].getRevisionTimestamp(),
        testResult.getRevisionTimestamp());
    assertEquals(
