...
import com.drupal.test.dao.DefaultLafOlQueueDao;
...
public class LafOlQueueHandlerTestIt {
  static final String inputFile = "LafOlQueue.json";
  static LafOlQueueHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlQueue.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlQueue[].class);
    assertEquals("match count", count, records.length);
    LafOlQueue testResult = jpa.find(LafOlQueue.class, records[0].getItemId());
