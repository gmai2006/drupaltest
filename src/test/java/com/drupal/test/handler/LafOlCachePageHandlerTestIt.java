...
import com.drupal.test.dao.DefaultLafOlCachePageDao;
...
public class LafOlCachePageHandlerTestIt {
  static final String inputFile = "LafOlCachePage.json";
  static LafOlCachePageHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCachePage.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCachePage[].class);
    assertEquals("match count", count, records.length);
    LafOlCachePage testResult = jpa.find(LafOlCachePage.class, records[0].getCid());
