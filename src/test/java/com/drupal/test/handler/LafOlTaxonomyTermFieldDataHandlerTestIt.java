...
import com.drupal.test.dao.DefaultLafOlTaxonomyTermFieldDataDao;
...
public class LafOlTaxonomyTermFieldDataHandlerTestIt {
  static final String inputFile = "LafOlTaxonomyTermFieldData.json";
  static LafOlTaxonomyTermFieldDataHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private LafOlTaxonomyTermFieldData[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlTaxonomyTermFieldData.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlTaxonomyTermFieldData[].class);
    assertEquals("match count", count, records.length);
    final LafOlTaxonomyTermFieldDataId id =
        new LafOlTaxonomyTermFieldDataId(this.records[0].getLangcode(), this.records[0].getTid());
    LafOlTaxonomyTermFieldData testResult = jpa.find(LafOlTaxonomyTermFieldData.class, id);
    org.junit.Assert.assertEquals(
        "expect equals revisionId ", this.records[0].getRevisionId(), testResult.getRevisionId());
    assertEquals("expect equals vid ", this.records[0].getVid(), testResult.getVid());
    org.junit.Assert.assertEquals(
