...
import com.drupal.test.dao.DefaultLafOlMenuLinkContentRevisionDao;
...
public class LafOlMenuLinkContentRevisionHandlerTestIt {
  static final String inputFile = "LafOlMenuLinkContentRevision.json";
  static LafOlMenuLinkContentRevisionHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlMenuLinkContentRevision.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlMenuLinkContentRevision[].class);
    assertEquals("match count", count, records.length);
    LafOlMenuLinkContentRevision testResult =
        jpa.find(LafOlMenuLinkContentRevision.class, records[0].getRevisionId());
