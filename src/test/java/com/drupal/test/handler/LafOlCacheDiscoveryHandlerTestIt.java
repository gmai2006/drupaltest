...
import com.drupal.test.dao.DefaultLafOlCacheDiscoveryDao;
...
public class LafOlCacheDiscoveryHandlerTestIt {
  static final String inputFile = "LafOlCacheDiscovery.json";
  static LafOlCacheDiscoveryHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private LafOlCacheDiscovery[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCacheDiscovery.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCacheDiscovery[].class);
    assertEquals("match count", count, records.length);
    LafOlCacheDiscovery testResult = jpa.find(LafOlCacheDiscovery.class, records[0].getCid());
