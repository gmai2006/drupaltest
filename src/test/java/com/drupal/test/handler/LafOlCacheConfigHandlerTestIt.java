...
import com.drupal.test.dao.DefaultLafOlCacheConfigDao;
...
public class LafOlCacheConfigHandlerTestIt {
  static final String inputFile = "LafOlCacheConfig.json";
  static LafOlCacheConfigHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCacheConfig.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCacheConfig[].class);
    assertEquals("match count", count, records.length);
    LafOlCacheConfig testResult = jpa.find(LafOlCacheConfig.class, records[0].getCid());
