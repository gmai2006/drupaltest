...
import com.drupal.test.dao.DefaultLafOlCacheDataDao;
...
public class LafOlCacheDataHandlerTestIt {
  static final String inputFile = "LafOlCacheData.json";
  static LafOlCacheDataHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private LafOlCacheData[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCacheData.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCacheData[].class);
    assertEquals("match count", count, records.length);
    LafOlCacheData testResult = jpa.find(LafOlCacheData.class, records[0].getCid());
