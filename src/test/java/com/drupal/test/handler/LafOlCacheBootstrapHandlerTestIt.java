...
import com.drupal.test.dao.DefaultLafOlCacheBootstrapDao;
...
public class LafOlCacheBootstrapHandlerTestIt {
  static final String inputFile = "LafOlCacheBootstrap.json";
  static LafOlCacheBootstrapHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private LafOlCacheBootstrap[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCacheBootstrap.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCacheBootstrap[].class);
    assertEquals("match count", count, records.length);
    LafOlCacheBootstrap testResult = jpa.find(LafOlCacheBootstrap.class, records[0].getCid());
