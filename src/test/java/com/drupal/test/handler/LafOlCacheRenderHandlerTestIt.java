...
import com.drupal.test.dao.DefaultLafOlCacheRenderDao;
...
public class LafOlCacheRenderHandlerTestIt {
  static final String inputFile = "LafOlCacheRender.json";
  static LafOlCacheRenderHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private LafOlCacheRender[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCacheRender.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCacheRender[].class);
    assertEquals("match count", count, records.length);
    LafOlCacheRender testResult = jpa.find(LafOlCacheRender.class, records[0].getCid());
