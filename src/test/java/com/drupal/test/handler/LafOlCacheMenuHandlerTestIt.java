...
import com.drupal.test.dao.DefaultLafOlCacheMenuDao;
...
public class LafOlCacheMenuHandlerTestIt {
  static final String inputFile = "LafOlCacheMenu.json";
  static LafOlCacheMenuHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private LafOlCacheMenu[] records;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCacheMenu.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, LafOlCacheMenu[].class);
    assertEquals("match count", count, records.length);
    LafOlCacheMenu testResult = jpa.find(LafOlCacheMenu.class, records[0].getCid());
