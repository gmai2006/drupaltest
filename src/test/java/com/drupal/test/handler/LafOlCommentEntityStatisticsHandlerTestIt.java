...
import com.drupal.test.dao.DefaultLafOlCommentEntityStatisticsDao;
...
public class LafOlCommentEntityStatisticsHandlerTestIt {
  static final String inputFile = "LafOlCommentEntityStatistics.json";
  static LafOlCommentEntityStatisticsHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/LafOlCommentEntityStatistics.csv");
    ...
    LafOlCommentEntityStatistics testResult = jpa.find(LafOlCommentEntityStatistics.class, id);
    org.junit.Assert.assertEquals(
        "expect equals cid ", this.records[0].getCid(), testResult.getCid());
    org.junit.Assert.assertEquals(
        "expect equals lastCommentTimestamp ",
        this.records[0].getLastCommentTimestamp(),
        testResult.getLastCommentTimestamp());
    assertEquals(
        "expect equals lastCommentName ",
        this.records[0].getLastCommentName(),
        testResult.getLastCommentName());
    org.junit.Assert.assertEquals(
        "expect equals lastCommentUid ",
        this.records[0].getLastCommentUid(),
        testResult.getLastCommentUid());
    org.junit.Assert.assertEquals(
        "expect equals commentCount ",
        this.records[0].getCommentCount(),
        testResult.getCommentCount());
