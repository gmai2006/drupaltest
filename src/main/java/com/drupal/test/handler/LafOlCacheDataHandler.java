...
// @Stateless
@Named("LafOlCacheDataHandler")
public class LafOlCacheDataHandler extends DelimiterFileHandler<LafOlCacheData> {

    @Inject
    @Named("DefaultJpaDao")
    public LafOlCacheDataHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected LafOlCacheData parseLine(List<String> headers, List<String> tokens) {
        LafOlCacheData record = new LafOlCacheData();
